...
func init() {
	...
	sepaCmd.Flags().BoolVar(&nonMembers, "nonMembers", false, "Also search for non Members (NICHT_MITGLIED)")
	sepaCmd.Flags().BoolVar(&schnupper, "schnupper", false, "Also search for Schnupper Members (SCHNUPPER_MITGLIED)")
	sepaCmd.Flags().BoolVar(&inactive, "inactive", false, "Search for inactive Members (INAKTIV)")
